...
def main():
    ...
    # Split operators with disjunctions into multiple operators
    new_task = copy.deepcopy(task)
    reformulation.task_modifier.split_disjunction_operators(new_task)


    
    with timers.timing("Normalizing task"):
        normalize.normalize(new_task)
    

    if options.syntax_check:
        new_task.syntax_check()


    # If there are any conditional effects, put them in the main body and calculate invariants
    taskToUseForInvariants, conditionalEffectGroups = reformulation.task_modifier.move_conditional_effects_into_body(new_task)
    (relaxed_reachable, atoms, actions, axioms, reachable_action_params) = instantiate.explore(taskToUseForInvariants)
    invariants = sorted(invariant_finder.find_invariants(taskToUseForInvariants, reachable_action_params))
    task = reformulation.transform(task, new_task, reachable_action_params, invariants, conditionalEffectGroups)
    ...
    print("Done! %s" % timer)
    print("Reformulation time: %s" % timers.Timer.just_clock(timer))
...
# python3 baggy.py --writeout_reformulated_pddl --writeout_reformulation_logic original-domain.pddl original-problem.pddl