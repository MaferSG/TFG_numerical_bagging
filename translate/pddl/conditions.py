...
class Literal(Condition):
    ...
    __hash__ = Condition.__hash__
    parts = []
    def __init__(self, predicate, args, type = None, number_objects = 0):
        self.predicate = predicate
        self.args = tuple(args)
        self.hash = hash((self.__class__, self.predicate, self.args))
        self.type = type # (greater, increase, decrease, assignation)
        self.number_objects = number_objects # Mafe
    def __eq__(self, other):
        # Compare hash first for speed reasons.
        return (self.hash == other.hash and
                self.__class__ is other.__class__ and
...
class Atom(Literal):
    ...
    def pddl_str(self):
        if self.type == 'greater':
            return '(>= (%s %s) 1)' % (self.predicate, ' '.join(map(str, self.args))) 
        elif self.type == 'increase':
            return '(increase (%s %s) 1)' % (self.predicate, ' '.join(map(str, self.args)))
        elif self.type == 'decrease':
            return '(decrease (%s %s) 1)' % (self.predicate, ' '.join(map(str, self.args)))
        elif self.type == 'assignation':
            return '(= (%s %s) %s)' % (self.predicate, ' '.join(map(str, self.args)), self.number_objects)
        else:
