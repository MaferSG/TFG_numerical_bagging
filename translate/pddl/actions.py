...
from reformulation import operator_bagger
...
class Action(object):
    ...
    def pddl_str(self):
        ...
        #Mafe: lo de operator_bagger.ActionFunction es para que se imprima bien cuando se usa la clase que hereda de Atom
        if self.precondition.parts != [] or isinstance(self.precondition, conditions.Atom) or isinstance(self.precondition, operator_bagger.Action_Function): 
            if [part.type for part in self.precondition.parts] == 'greater':
                result += '\t:precondition ' + self.precondition.pddl_str() + '\n'
                # print([part.type for part in self.precondition.parts])
            else:
                result += '\t:precondition ' + self.precondition.pddl_str() + '\n'
